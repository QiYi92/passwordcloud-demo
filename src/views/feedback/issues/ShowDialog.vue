...
const canDownload = computed(() => {
  const fileField = formattedData.value.find(
    item => item.field === "issues_files"
  );
  return fileField && fileField.value !== "0";
});
...
const handleDownload = () => {
  ...
  if (fileField && fileField.value !== "0") {
    const baseUrl = import.meta.env.VITE_APP_SERVER; // 服务器的 base URL
    const fileUrl = `${baseUrl}/uploads/issues/${fileField.value.replace(/^\//, "")}`;
    // 创建一个隐藏的 <a> 元素用于下载
